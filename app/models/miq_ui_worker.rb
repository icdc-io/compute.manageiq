class MiqUiWorker < MiqWorker
  ...
  def mount_configmap(container_definition)
    ...
    container = container_definition[:spec][:template][:spec][:containers].first
    container.merge!{:volumeMounts => [{ :mountPath => '/etc/httpd/conf', :name => ENV['CUSTOM_CONFIG_NAME'], :readOnly => true }]}
  end
end
