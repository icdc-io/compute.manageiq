module ServiceScheduleMixin
  ...
  def schedule_power_op(params)
    sched = self.miq_schedules.create(
        :name => "service #{name} power #{params["power"]} scheduled at #{params["start_time"]}",
        :description => "power #{params["power"]}",
        :sched_action => {
            method: "service_power_op",
            options: params.slice("power")
        },
        :filter => MiqExpression.new("=" => {"field" => "Service-id", "value" => self.id}),
        :towhat => self.class.name,
        :run_at => {
            :interval => {unit: params['interval_unit'], value: '1'},
  ...
  def backup_retention_period
    @backup_retention_period ||= self.backup_retention_attr&.value || DEFAULT_BACKUP_RETENTION_PERIOD
  end

  def backup_retention_period=(value)

    attr = self.backup_retention_attr || self.create_default_backup_retention_period
  ...
  def backup_retention_attr
    @backup_retention_attr ||= self.custom_attributes.find_by(name: BACKUP_RETENTION_NAME)
  end

  def create_default_backup_retention_period
    self.custom_attributes.create(name: BACKUP_RETENTION_NAME, value: DEFAULT_BACKUP_RETENTION_PERIOD)
  end
